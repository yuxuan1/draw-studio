...
import type {
  RenderNode, ProjectedEdge, Handle, NodeType, Position, ID, CallNode, MiniLayout,
} from '../lib/domain';
import { NODE_DEFAULTS, layoutMini } from '../lib/domain';
...
export default function ScopeCanvas() {
  ...
  useEffect(() => {
    const key = app.scope.pageId + ':' + (app.scope.processId ?? '');
      ...
      <svg ref={svgRef} className="w-full h-full block" style={{ cursor: tool === 'select' ? 'default' : 'crosshair', touchAction: 'none' }}
        ...
        <g transform={`translate(${view.x} ${view.y}) scale(${view.k})`}>
          ...
          {projection.nodes.map((n) => (
            <NodeView key={n.id} n={n} theme={theme}
...
/* ---------------- 节点 ---------------- */
function NodeView({ n, theme, selected, hovered, previewOpen, onHover, onDown, onClick, onDblClick, onStartConnect }: {
  n: RenderNode; theme: ThemeMode; selected: boolean; hovered: boolean; previewOpen: boolean;
  ...
  return (
    <g data-el="1" {...common}>
      ...
      {/* 四锚点 */}
      {(hovered || selected) && HANDLE_IDS.map((h) => {
